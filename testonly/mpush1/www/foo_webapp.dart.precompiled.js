...
Interceptor: {"": "Object;",
  ...
  "%": "CanvasGradient|CanvasPattern|DOMError|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedNumberList|SVGAnimatedString"
},
...
InputElement: {"": "HtmlElement;height=,size=,src},width=", $isEventTarget: true, "%": "HTMLInputElement"},
...
FEBlendElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEBlendElement"},

FEColorMatrixElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEColorMatrixElement"},

FEComponentTransferElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEComponentTransferElement"},

FECompositeElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFECompositeElement"},

FEConvolveMatrixElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEConvolveMatrixElement"},

FEDiffuseLightingElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEDiffuseLightingElement"},

FEDisplacementMapElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEDisplacementMapElement"},

FEFloodElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEFloodElement"},

FEGaussianBlurElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEGaussianBlurElement"},

FEImageElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEImageElement"},

FEMergeElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEMergeElement"},

FEMorphologyElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEMorphologyElement"},

FEOffsetElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFEOffsetElement"},

FESpecularLightingElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFESpecularLightingElement"},

FETileElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFETileElement"},

FETurbulenceElement: {"": "SvgElement;height=,result=,width=", "%": "SVGFETurbulenceElement"},
...
ByteBuffer: {"": "Interceptor;lengthInBytes:byteLength=", "%": "ArrayBuffer"},

TypedData: {"": "Interceptor;lengthInBytes:byteLength=",
...
outputMsg: function(msg) {
  ...
  t1 = $.count;
  $.count = t1 + 1;
  if (C.JSInt_methods.$mod(t1, $.queueSize) === 0)
    $.get$output().textContent = "";
  text = $.get$output().textContent.length !== 0 ? msg + "\n" + $.get$output().textContent : msg;
  $.get$output().textContent = text;
},

drawRecArrayByIntList: function(ce, rgbs) {
  var context, w, h, t1, i, t2, t3, t4;
  context = J.getContext$1$x(ce, "2d");
  w = ce.width;
  h = ce.height;
  ...
  t1 = J.getInterceptor$asx(rgbs);
  i = 0;
  while (true) {
    t2 = t1.get$length(rgbs);
    if (typeof t2 !== "number")
      throw H.iae(t2);
    if (!(i < t2))
      break;
    ...
    if (typeof t2 !== "number")
      throw t2.$shr();
    t2 = C.JSNumber_methods.$shr(t2, 16);
    t3 = t1.$index(rgbs, i);
    if (typeof t3 !== "number")
      throw t3.$shr();
    t3 = C.JSNumber_methods.$shr(t3, 8);
    t4 = t1.$index(rgbs, i);
    if (typeof t4 !== "number")
      throw t4.$and();
    context.fillStyle = "rgb(" + (t2 & 255) + "," + (t3 & 255) + "," + (t4 & 255) + ")";
    context.fillRect(C.JSInt_methods.$mod(i, 100) * 2, C.JSNumber_methods.toInt$0(Math.floor(i / 100)) * 2, 2, 2);
    ++i;
  }
  context.stroke();
  context.closePath();
...
drawRecRgbUint8List: function(w, h, context, rgbs) {
  var size, i, t1, t2, r, t3, g, b;
  J.beginPath$0$x(context);
  context.clearRect(0, 0, w, h);
  context.fillStyle = "#ffffff";
  context.strokeStyle = "#000000";
  context.fillRect(0, 0, w, h);
  size = C.JSInt_methods.$tdiv(C.JS_CONST_ZYJ(rgbs), 3);
  for (i = 0; i < size; ++i) {
    t1 = i * 3;
    t2 = C.JS_CONST_ZYJ(rgbs);
    if (t1 < 0 || t1 >= t2)
      throw H.ioore(rgbs, t1);
    r = rgbs[t1];
    t3 = t1 + 1;
    if (t3 >= t2)
      throw H.ioore(rgbs, t3);
    g = rgbs[t3];
    t1 += 2;
    if (t1 >= t2)
      throw H.ioore(rgbs, t1);
    ...
    context.fillRect(C.JSInt_methods.$mod(i, w), C.JSNumber_methods.toInt$0(Math.floor(i / w)), 1, 1);
  }
  context.stroke();
  context.closePath();
},

drawDiffArray: function(max) {
  var w, h, t1, r, i;
  w = J.get$width$x($.get$canvasQ());
  h = J.get$height$x($.get$canvasQ());
  J.beginPath$0$x($.get$contextQ());
  J.clearRect$4$x($.get$contextQ(), 0, 0, w, h);
  J.set$fillStyle$x($.get$contextQ(), "#ffffff");
  J.set$strokeStyle$x($.get$contextQ(), "#000000");
  J.fillRect$4$x($.get$contextQ(), 0, 0, w, h);
  J.set$fillStyle$x($.get$contextQ(), "#ff0000");
  t1 = $.get$dataQueue();
  r = t1.toList$1$growable(t1, false);
  for (i = 0; i < r.length; ++i) {
    t1 = r[i];
    if (typeof t1 !== "number")
      throw t1.$div();
    t1 = C.JSNumber_methods.toInt$0(Math.floor(t1 / max * 100 * -1));
    J.fillRect$4$x($.get$contextQ(), i * 4, 95, 4, t1 - 1);
  }
  J.stroke$0$x($.get$contextQ());
  J.closePath$0$x($.get$contextQ());
},
...
initWebSocket: function(retrySeconds) {
  ...
  H.setRuntimeTypeInfo(t1, [null]);
  t2 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new G.initWebSocket_closure2(new G.initWebSocket_loadBlobRgbArrayBuf(), new G.initWebSocket_loadBlobJpegReadUrl())), t1._useCapture);
...
initWebSocket_loadBlobRgbArrayBuf: {"": "Closure;",
  call$1: function(b) {
    var fileReader, t1, t2;
    fileReader = new FileReader();
    P.print("Read Blob/ArrayBuffer size = " + H.S(J.get$size$x(b)));
    t1 = new W._EventStream(fileReader, C.EventStreamProvider_loadend._eventType, false);
    H.setRuntimeTypeInfo(t1, [null]);
    t2 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new G.initWebSocket_loadBlobRgbArrayBuf_closure()), t1._useCapture);
    H.setRuntimeTypeInfo(t2, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
    t2._tryResume$0();
...
initWebSocket_loadBlobRgbArrayBuf_closure: {"": "Closure;",
  call$1: function(evt) {
    var k, t1, buffer;
    k = J.get$result$x(J.get$target$x(evt));
    t1 = J.get$lengthInBytes$x(k);
    if (t1 == null) {
      buffer = new Uint8Array(k, 0);
      buffer.$dartCachedLength = buffer.length;
    } else {
      buffer = new Uint8Array(k, 0, t1);
      buffer.$dartCachedLength = buffer.length;
    }
    G.drawRecRgbUint8List(100, 100, $.get$context4(), buffer);
  },
  $is_args1: true
},

initWebSocket_loadBlobJpegReadUrl: {"": "Closure;",
  call$1: function(b) {
    var fileReader, t1, t2;
    fileReader = new FileReader();
    t1 = new W._EventStream(fileReader, C.EventStreamProvider_loadend._eventType, false);
    H.setRuntimeTypeInfo(t1, [null]);
    t2 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new G.initWebSocket_loadBlobJpegReadUrl_closure()), t1._useCapture);
    H.setRuntimeTypeInfo(t2, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
    t2._tryResume$0();
...
initWebSocket_loadBlobJpegReadUrl_closure: {"": "Closure;",
  call$1: function(evt) {
    J.set$src$x($.get$myimg(), J.get$result$x(J.get$target$x(evt)));
  },
  $is_args1: true
},

initWebSocket_closure2: {"": "Closure;loadBlobRgbArrayBuf_6,loadBlobJpegReadUrl_7",
  call$1: function(e) {
    var t1, t2, t3, rlist, mlist;
    t1 = "msg " + $.count + " : is String ? = ";
    t2 = J.getInterceptor$x(e);
    t3 = t2.get$data(e);
    G.outputMsg(t1 + (typeof t3 === "string"));
    t1 = t2.get$data(e);
    if (typeof t1 === "string") {
      rlist = C.C_JsonCodec.decode$1(t2.get$data(e));
      t1 = J.getInterceptor$asx(rlist);
      G.outputMsg("msg " + $.count + " : v=" + H.S(t1.get$length(rlist)));
      if (t1.get$length(rlist) === 1) {
        $.cmdId = t1.$index(rlist, 0);
        P.print("Read CmdId = " + H.S($.cmdId));
      } else if (t1.get$length(rlist) === 2) {
        mlist = t1.$index(rlist, 1);
        $.get$dataQueue()._add$1(t1.$index(rlist, 0));
        t1 = $.get$dataQueue();
        if (t1.get$length(t1) > $.get$lineUp().length)
          $.get$dataQueue().removeFirst$0();
        G.drawDiffArray(10000);
        if ($.showRgbJsonDiff)
          G.drawRecArrayByIntList($.get$canvasM(), mlist);
      } else if (t1.get$length(rlist) === 10000)
        if ($.showRgbJsonImg)
          G.drawRecArrayByIntList($.get$canvas(), rlist);
    } else if (J.$eq($.cmdId, 5)) {
      if ($.showJpgBinSrc)
        this.loadBlobJpegReadUrl_7.call$1(t2.get$data(e));
    } else if (J.$eq($.cmdId, 6))
      if ($.showRgbBinImg)
        this.loadBlobRgbArrayBuf_6.call$1(t2.get$data(e));
  },
  $is_args1: true
},
...
Isolate.$lazy($, "myimg", "myimg", "get$myimg", function() {
  return document.querySelector("#myimg");
});
Isolate.$lazy($, "canvas", "canvas", "get$canvas", function() {
  return document.querySelector("#canvas");
});
Isolate.$lazy($, "canvasM", "canvasM", "get$canvasM", function() {
  return document.querySelector("#canvasM");
});
Isolate.$lazy($, "canvasQ", "canvasQ", "get$canvasQ", function() {
  return document.querySelector("#canvasQ");
});
Isolate.$lazy($, "canvas4", "canvas4", "get$canvas4", function() {
  return document.querySelector("#canvas4");
});
Isolate.$lazy($, "contextQ", "contextQ", "get$contextQ", function() {
  return J.getContext$1$x($.get$canvasQ(), "2d");
});
Isolate.$lazy($, "context4", "context4", "get$context4", function() {
  return J.getContext$1$x($.get$canvas4(), "2d");
});
Isolate.$lazy($, "lineUp", "lineUp", "get$lineUp", function() {
  var t1 = new P._GeneratorIterable($.queueSize, new G.closure());
  H.setRuntimeTypeInfo(t1, [null]);
...
function dart_precompiled($collectedClasses) {
  ...
  FileReader.prototype.get$result = function(receiver) {
  ...
  ByteBuffer.prototype.get$lengthInBytes = function(receiver) {
  ...
  TypedData.prototype.get$lengthInBytes = function(receiver) {
    return receiver.byteLength;
  };
  function ByteData() {
  }
  ...
  function initWebSocket_loadBlobRgbArrayBuf() {
  }
  initWebSocket_loadBlobRgbArrayBuf.builtin$cls = "initWebSocket_loadBlobRgbArrayBuf";
  if (!"name" in initWebSocket_loadBlobRgbArrayBuf)
    initWebSocket_loadBlobRgbArrayBuf.name = "initWebSocket_loadBlobRgbArrayBuf";
  $desc = $collectedClasses.initWebSocket_loadBlobRgbArrayBuf;
  if ($desc instanceof Array)
    $desc = $desc[1];
  initWebSocket_loadBlobRgbArrayBuf.prototype = $desc;
  function initWebSocket_loadBlobRgbArrayBuf_closure() {
  }
  initWebSocket_loadBlobRgbArrayBuf_closure.builtin$cls = "initWebSocket_loadBlobRgbArrayBuf_closure";
  if (!"name" in initWebSocket_loadBlobRgbArrayBuf_closure)
    initWebSocket_loadBlobRgbArrayBuf_closure.name = "initWebSocket_loadBlobRgbArrayBuf_closure";
  $desc = $collectedClasses.initWebSocket_loadBlobRgbArrayBuf_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  initWebSocket_loadBlobRgbArrayBuf_closure.prototype = $desc;
  function initWebSocket_loadBlobJpegReadUrl() {
  }
  initWebSocket_loadBlobJpegReadUrl.builtin$cls = "initWebSocket_loadBlobJpegReadUrl";
  if (!"name" in initWebSocket_loadBlobJpegReadUrl)
    initWebSocket_loadBlobJpegReadUrl.name = "initWebSocket_loadBlobJpegReadUrl";
  $desc = $collectedClasses.initWebSocket_loadBlobJpegReadUrl;
  if ($desc instanceof Array)
    $desc = $desc[1];
  initWebSocket_loadBlobJpegReadUrl.prototype = $desc;
  function initWebSocket_loadBlobJpegReadUrl_closure() {
  }
  initWebSocket_loadBlobJpegReadUrl_closure.builtin$cls = "initWebSocket_loadBlobJpegReadUrl_closure";
  if (!"name" in initWebSocket_loadBlobJpegReadUrl_closure)
    initWebSocket_loadBlobJpegReadUrl_closure.name = "initWebSocket_loadBlobJpegReadUrl_closure";
  $desc = $collectedClasses.initWebSocket_loadBlobJpegReadUrl_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  initWebSocket_loadBlobJpegReadUrl_closure.prototype = $desc;
  function initWebSocket_closure2(loadBlobRgbArrayBuf_6, loadBlobJpegReadUrl_7) {
    this.loadBlobRgbArrayBuf_6 = loadBlobRgbArrayBuf_6;
    this.loadBlobJpegReadUrl_7 = loadBlobJpegReadUrl_7;
  }
